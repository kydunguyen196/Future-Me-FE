...
interface ReadingWritingTestScreenProps {
  partNumber: number;
  questions: Question[];
...
/**
 * Reusable Reading & Writing test screen component
 */
const ReadingWritingTestScreen: React.FC<ReadingWritingTestScreenProps> = ({
  partNumber,
  questions,
  timeInMinutes,
  examId,
  ...
  if (showCheckYourWork) {
    const questionsForReview = questions.map((q, index) => ({
      ...
      module: `Reading & Writing: Module ${partNumber}`
    }));

    return (
      <CheckYourWorkScreen
        moduleTitle={`Reading & Writing: Module ${partNumber}`}
  ...
  // console.log('ReadingWritingTestScreen - Current question:', {
    ...
    <div className="test-passage-column h-full">
      <div className="prose prose-lg max-w-none">
        {currentQuestion.type === 'TEXT' ? (
          ...
          <div>
            <LatexRenderer content={currentQuestion.text} block={true} isMathModule={false} />
            {currentQuestion.image && (
              <div className="mt-6">
                <img 
                  src={currentQuestion.image} 
      ...
      <TestQuestionLayout
        ...
        questionContent={
          <div className="test-question-column h-full">
            ...
            <SatTestQuestion
              question={{
                questionText: "", // Don't repeat the question text here since it's on the left
...
export default ReadingWritingTestScreen; 